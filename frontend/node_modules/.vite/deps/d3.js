...
import {
  ...
  viridis_default,
  warm
} from "./chunk-FPL4G3GD.js";
import "./chunk-LHDATKDH.js";
import {
  Adder,
  FormatSpecifier,
  InternMap,
...
function init(node, id3) {
  var schedule = get(node, id3);
  if (schedule.state > CREATED) throw new Error("too late; already scheduled");
  return schedule;
}
function set(node, id3) {
  var schedule = get(node, id3);
  if (schedule.state > STARTED) throw new Error("too late; already running");
  return schedule;
}
function get(node, id3) {
...
function tweenRemove(id3, name) {
  var tween0, tween1;
  return function() {
    var schedule = set(this, id3), tween = schedule.tween;
    if (tween !== tween0) {
      tween1 = tween0 = tween;
...
function tween_default(name, value) {
  ...
  name += "";
  if (arguments.length < 2) {
    var tween = get(this.node(), id3).tween;
...
function tweenValue(transition3, name, value) {
  ...
  transition3.each(function() {
    var schedule = set(this, id3);
    (schedule.value || (schedule.value = {}))[name] = value.apply(this, arguments);
  });
  return function(node) {
    return get(node, id3).value[name];
  };
}
...
function delay_default(value) {
  var id3 = this._id;
  return arguments.length ? this.each((typeof value === "function" ? delayFunction : delayConstant)(id3, value)) : get(this.node(), id3).delay;
}

// node_modules/d3-brush/node_modules/d3-transition/src/transition/duration.js
function durationFunction(id3, value) {
  return function() {
    set(this, id3).duration = +value.apply(this, arguments);
  };
}
function durationConstant(id3, value) {
  return value = +value, function() {
    set(this, id3).duration = value;
  };
}
function duration_default(value) {
  var id3 = this._id;
  return arguments.length ? this.each((typeof value === "function" ? durationFunction : durationConstant)(id3, value)) : get(this.node(), id3).duration;
}
...
function easeConstant(id3, value) {
  if (typeof value !== "function") throw new Error();
  return function() {
    set(this, id3).ease = value;
  };
}
function ease_default(value) {
  var id3 = this._id;
  return arguments.length ? this.each(easeConstant(id3, value)) : get(this.node(), id3).ease;
}
...
function on_default2(name, listener) {
  var id3 = this._id;
  return arguments.length < 2 ? get(this.node(), id3).on.on(name) : this.each(onFunction(id3, name, listener));
}
  ...
  for (var groups2 = this._groups, m3 = groups2.length, subgroups = new Array(m3), j = 0; j < m3; ++j) {
    for (var group2 = groups2[j], n = group2.length, subgroup = subgroups[j] = new Array(n), node, subnode, i = 0; i < n; ++i) {
      if ((node = group2[i]) && (subnode = select.call(node, node.__data__, i, group2))) {
        if ("__data__" in node) subnode.__data__ = node.__data__;
        subgroup[i] = subnode;
        schedule_default(subgroup[i], name, id3, i, subgroup, get(node, id3));
      }
    }
  }
...
function selectAll_default3(select) {
  ...
  for (var groups2 = this._groups, m3 = groups2.length, subgroups = [], parents = [], j = 0; j < m3; ++j) {
    for (var group2 = groups2[j], n = group2.length, node, i = 0; i < n; ++i) {
      if (node = group2[i]) {
        for (var children3 = select.call(node, node.__data__, i, group2), child, inherit3 = get(node, id3), k = 0, l = children3.length; k < l; ++k) {
...
function styleMaybeRemove(id3, name) {
  ...
  return function() {
    var schedule = set(this, id3), on = schedule.on, listener = schedule.value[key] == null ? remove3 || (remove3 = styleRemove2(name)) : void 0;
...
function end_default() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    that.each(function() {
      var schedule = set(this, id3), on = schedule.on;
      if (on !== on0) {
        on1 = (on0 = on).copy();
...
function brush(dim) {
  var extent2 = defaultExtent, filter4 = defaultFilter, touchable = defaultTouchable, keys = true, listeners = dispatch_default("start", "brush", "end"), handleSize = 6, touchending;
  ...
  function started(event) {
    ...
    var that = this, type2 = event.target.__data__.type, mode2 = (keys && event.metaKey ? type2 = "overlay" : type2) === "selection" ? MODE_DRAG : keys && event.altKey ? MODE_CENTER : MODE_HANDLE, signX = dim === Y ? null : signsX[type2], signY = dim === X ? null : signsY[type2], state = local2(that), extent3 = state.extent, selection3 = state.selection, W = extent3[0][0], w0, w1, N = extent3[0][1], n0, n1, E = extent3[1][0], e0, e1, S = extent3[1][1], s0, s1, dx = 0, dy = 0, moving, shifting = signX && signY && keys && event.shiftKey, lockX, lockY, points = Array.from(event.touches || [event], (t) => {
    ...
    function move(event2) {
      ...
      if (state.selection) selection3 = state.selection;
      if (lockX) w1 = selection3[0][0], e1 = selection3[1][0];
      if (lockY) n1 = selection3[0][1], s1 = selection3[1][1];
    ...
    function ended(event2) {
      ...
      if (state.selection) selection3 = state.selection;
      if (empty2(selection3)) state.selection = null, redraw.call(that);
  ...
  brush2.filter = function(_) {
    return arguments.length ? (filter4 = typeof _ === "function" ? _ : constant_default2(!!_), brush2) : filter4;
  };
...
function contours_default() {
  ...
  function isorings(values, value, callback) {
    ...
    function stitch(line) {
      var start3 = [line[0][0] + x3, line[0][1] + y3], end = [line[1][0] + x3, line[1][1] + y3], startIndex = index3(start3), endIndex = index3(end), f, g;
...
function selectAll_default5(selector) {
  return typeof selector === "string" ? new Selection3([document.querySelectorAll(selector)], [document.documentElement]) : new Selection3([array3(selector)], root2);
}
...
function nodrag_default2(view) {
  var root4 = view.document.documentElement, selection3 = select_default5(view).on("dragstart.drag", noevent_default2, nonpassivecapture);
...
function drag_default() {
  var filter4 = defaultFilter2, container = defaultContainer, subject = defaultSubject, touchable = defaultTouchable2, gestures = {}, listeners = dispatch_default3("start", "drag", "end"), active = 0, mousedownx, mousedowny, mousemoving, touchending, clickDistance2 = 0;
  ...
  function mousedowned(event, d) {
    ...
    if (!gesture) return;
    select_default5(event.view).on("mousemove.drag", mousemoved, nonpassivecapture).on("mouseup.drag", mouseupped, nonpassivecapture);
  ...
  function beforestart(that, container2, event, d, identifier, touch) {
    var dispatch2 = listeners.copy(), p = pointer_default2(touch || event, container2), dx, dy, s;
  ...
  drag.filter = function(_) {
    return arguments.length ? (filter4 = typeof _ === "function" ? _ : constant_default6(!!_), drag) : filter4;
  };
  drag.container = function(_) {
    return arguments.length ? (container = typeof _ === "function" ? _ : constant_default6(_), drag) : container;
  };
  drag.subject = function(_) {
    return arguments.length ? (subject = typeof _ === "function" ? _ : constant_default6(_), drag) : subject;
  };
  drag.touchable = function(_) {
    return arguments.length ? (touchable = typeof _ === "function" ? _ : constant_default6(!!_), drag) : touchable;
  };
  drag.on = function() {
    var value = listeners.on.apply(listeners, arguments);
...
function parser(type2) {
  return (input, init3) => text_default4(input, init3).then((text) => new DOMParser().parseFromString(text, type2));
}
...
function link_default(links) {
  var id3 = index2, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count3, bias, random, iterations2 = 1;
  if (links == null) links = [];
...
function leafLeft(node) {
  var children3;
  while (children3 = node.children) node = children3[0];
  return node;
}
function leafRight(node) {
  var children3;
  while (children3 = node.children) node = children3[children3.length - 1];
  return node;
}
...
function count2(node) {
  ...
  if (!i) sum2 = 1;
  else while (--i >= 0) sum2 += children3[i].value;
  node.value = sum2;
}
...
function sum_default(value) {
  return this.eachAfter(function(node) {
    ...
    while (--i >= 0) sum2 += children3[i].value;
    node.value = sum2;
  });
}
...
function packChildrenRandom(padding, k, random) {
  return function(node) {
    if (children3 = node.children) {
      var children3, i, n = children3.length, r = padding(node) * k || 0, e;
      if (r) for (i = 0; i < n; ++i) children3[i].r += r;
  ...
  function stratify(data) {
    ...
    for (i = 0; i < n; ++i) {
      ...
      if (nodeId = node.parent) {
        ...
        if (root4) throw new Error("multiple roots");
        root4 = node;
      }
    }
    if (!root4) throw new Error("no root");
    if (path2 != null) {
...
function tree_default() {
  ...
  function tree(root4) {
    ...
    if (nodeSize) root4.eachBefore(sizeNode);
    else {
...
function delay_default2(value) {
  var id3 = this._id;
  return arguments.length ? this.each((typeof value === "function" ? delayFunction2 : delayConstant2)(id3, value)) : get3(this.node(), id3).delay;
}
...
function duration_default2(value) {
  var id3 = this._id;
  return arguments.length ? this.each((typeof value === "function" ? durationFunction2 : durationConstant2)(id3, value)) : get3(this.node(), id3).duration;
}
...
function ease_default2(value) {
  var id3 = this._id;
  return arguments.length ? this.each(easeConstant2(id3, value)) : get3(this.node(), id3).ease;
}
...
function on_default4(name, listener) {
  var id3 = this._id;
  return arguments.length < 2 ? get3(this.node(), id3).on.on(name) : this.each(onFunction2(id3, name, listener));
}
  ...
  for (var groups2 = this._groups, m3 = groups2.length, subgroups = new Array(m3), j = 0; j < m3; ++j) {
    for (var group2 = groups2[j], n = group2.length, subgroup = subgroups[j] = new Array(n), node, subnode, i = 0; i < n; ++i) {
      if ((node = group2[i]) && (subnode = select.call(node, node.__data__, i, group2))) {
        if ("__data__" in node) subnode.__data__ = node.__data__;
        subgroup[i] = subnode;
        schedule_default2(subgroup[i], name, id3, i, subgroup, get3(node, id3));
      }
    }
  }
...
function selectAll_default6(select) {
  ...
  for (var groups2 = this._groups, m3 = groups2.length, subgroups = [], parents = [], j = 0; j < m3; ++j) {
    for (var group2 = groups2[j], n = group2.length, node, i = 0; i < n; ++i) {
      if (node = group2[i]) {
        for (var children3 = select.call(node, node.__data__, i, group2), child, inherit3 = get3(node, id3), k = 0, l = children3.length; k < l; ++k) {
          if (child = children3[k]) {
            schedule_default2(child, name, id3, k, children3, inherit3);
          }
        }
        subgroups.push(children3);
        parents.push(node);
...
function styleMaybeRemove2(id3, name) {
  var on0, on1, listener0, key = "style." + name, event = "end." + key, remove3;
  return function() {
    var schedule = set3(this, id3), on = schedule.on, listener = schedule.value[key] == null ? remove3 || (remove3 = styleRemove4(name)) : void 0;
    if (on !== on0 || listener0 !== listener) (on1 = (on0 = on).copy()).on(event, listener0 = listener);
    schedule.on = on1;
  };
}
...
function zoom_default2() {
  var filter4 = defaultFilter3, extent2 = defaultExtent2, constrain = defaultConstrain, wheelDelta = defaultWheelDelta, touchable = defaultTouchable3, scaleExtent = [0, Infinity], translateExtent = [[-Infinity, -Infinity], [Infinity, Infinity]], duration = 250, interpolate = zoom_default, listeners = dispatch_default3("start", "zoom", "end"), touchstarting, touchfirst, touchending, touchDelay = 500, wheelDelay = 150, clickDistance2 = 0, tapDistance = 10;
  ...
  zoom.translateBy = function(selection3, x3, y3, event) {
    zoom.transform(selection3, function() {
  ...
  zoom.translateTo = function(selection3, x3, y3, p, event) {
    zoom.transform(selection3, function() {
  ...
  function schedule(transition3, transform2, point2, event) {
  ...
  function mousedowned(event, ...args) {
    ...
    var currentTarget = event.currentTarget, g = gesture(this, args, true).event(event), v = select_default5(event.view).on("mousemove.zoom", mousemoved, true).on("mouseup.zoom", mouseupped, true), p = pointer_default2(event, currentTarget), x06 = event.clientX, y06 = event.clientY;
    nodrag_default2(event.view);
  ...
  function dblclicked(event, ...args) {
    if (!filter4.apply(this, arguments)) return;
    var t0 = this.__zoom, p02 = pointer_default2(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t0.invert(p02), k1 = t0.k * (event.shiftKey ? 0.5 : 2), t1 = constrain(translate(scale(t0, k1), p02, p1), extent2.apply(this, args), translateExtent);
    noevent_default3(event);
    if (duration > 0) select_default5(this).transition().duration(duration).call(schedule, t1, p02, event);
    else select_default5(this).call(zoom.transform, t1, p02, event);
  }
  ...
  zoom.filter = function(_) {
    return arguments.length ? (filter4 = typeof _ === "function" ? _ : constant_default10(!!_), zoom) : filter4;
  };
