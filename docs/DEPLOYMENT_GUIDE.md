...
- 🛡️ **API Proxy**: CloudFront routes all `/api/*` traffic to the Application Load Balancer (ALB) via an HTTP Origin, completely shielding backend resources from the open internet.
...
- 🏗️ **`InsuranceInsightAppStack`**: Provisions the VPC, Subnets, Fargate Cluster, Load Balancer, S3 Bucket, CloudFront Distribution, DynamoDB Audit Table, and IAM Roles with strict least-privilege policies.

---

## 🛠️ 3. Prerequisite Tooling
...
### 🏎️ What happens during the script?
...
6. 🏗️ Deploys the mighty App Stack.
7. ♻️ Forces an ECS service update to cycle any existing tasks.
8. ⚛️ Compiles the React Vite frontend.
